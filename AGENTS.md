...
## Layout and Architectural Decisions (June 2026)

### 1. Modern Glassmorphism Header
- Implemented a sticky header with a backdrop filter (`backdrop-blur-md bg-[#0a0a0f]/80` and bottom border `border-white/10`).
- Grouped controls logically:
  - Brand logo / Title on the left.
  - Active audio input toggle (`MicToggle`) and visual level indicator (`AudioLevelMeter`) on the right in a unified, inner-shadowed card component.
  - Noise gate / sensitivity slider (`SensitivitySlider`) constrained to a maximum width to prevent stretching.
  - Theme selector (`ThemeSelector`) housed in its own compact wrapper card.

### 2. Central Note HUD Visualizer
- Created a centered HUD visualizer card in the main area above the keyboard to fill the desktop empty space.
- Shows active note names in large, striking typography (6xl/7xl bold with neon glow/drop shadow using theme CSS variables), exact frequency to 2 decimal places, and signal clarity percentage.
- Features a standby pulsing indicator when no input is active.

### 3. Keyboard Container and Sizing
- Constrained the keyboard container to `max-w-[1400px]` and centered it on desktop screens, adding `shadow-2xl` and a `border-4 border-[#1e1e2d]` to mimic a physical piano instrument keybed.
- Addressed key squishing on small/mobile viewports by setting a minimum width of `1200px` for keyboard geometry rendering, causing it to overflow and scroll horizontally (`overflow-x-auto`) inside the container.
- Adjusted coordinates and dimensions calculated by `getAllKeyGeometry` to use `layoutWidth` (minimum `1200px`) while keeping smooth-scrolling to active key logic (`scrollTo`) working properly relative to the scrolling container.

### 4. Note History Panel
- Refactored the note history panel into a glassmorphic bar.
